"""Nova bootstrap interpreter — runs Natural-syntax AST."""
...
def _suggest(name, candidates):
    """Did-you-mean: closest candidate or empty string."""
...
class NothingSignal(Exception):
    """C03: absence-of-value under a '?' expression. Swallowed ONLY by QuestionE-
    boundary; uncaught it becomes a friendly sentence + fix hint (error_handling.md 2.1)."""
...
class ModuleInstance:
    ...
    def __init__(self, name, path):
        ...
        self.funcs = {}
        self.things = {}
        self.scope = Scope()  # parent=None: complete isolation from the main program
...
class Scope:
    ...
    def get(self, name, line):
        ...
        while s:
            ...
            s = s.parent
        raise NovaError(line, f"the variable '{name}' does not exist{_suggest(name, known)}"
                              " — check the spelling or declare it with 'x is ...'")
...
def _read_text_file(path, line):
    try:
        ...
        raise NovaError(line, f"'{path}' does not exist")
    except IsADirectoryError:
        raise NovaError(line, f"'{path}' is a directory, not a file")
    except UnicodeDecodeError:
        raise NovaError(line, f"'{path}' is not a UTF-8 text file")
    except OSError as err:
        raise NovaError(line, f"cannot read '{path}': {err}")


def _bi_json_parse(args, line):
    v = args[0]
    if not isinstance(v, str):
        raise NovaError(line, "'json.parse' requires text — give it a string containing json")
    try:
        return _json.loads(v)
    except _json.JSONDecodeError as err:
        raise NovaError(line, f"invalid json (line {err.lineno}) — check the text, "
                              "or catch the failure with 'try ... if it fails'")
...
def _bi_file_write(args, line):
    ...
    if not isinstance(text, str):
        raise NovaError(line, "'file.write' requires text as the content")
    try:
        with open(path, "w", encoding="utf-8") as f:
            f.write(text)
    except OSError as err:
        raise NovaError(line, f"cannot save to '{path}': {err}")
    return NOTHING
...
def _bi_random_between(args, line):
    ...
    if not (_is_num(a) and _is_num(b)):
        raise NovaError(line, "'random.between' requires two numbers")
...
def _bi_random_pick(args, line):
    ...
    if not isinstance(xs, list) or not xs:
        raise NovaError(line, "'random.pick' requires a non-empty list")
...
def _bi_time_sleep(args, line):
    s = args[0]
    if not _is_num(s):
        raise NovaError(line, "'time.sleep' requires a number (seconds)")
    if s < 0:
        raise NovaError(line, "'time.sleep' sleep for a negative number of seconds")
    time.sleep(float(s))
    return NOTHING
...
def _bi_math_sqrt(args, line):
    ...
    if not _is_num(n) or n < 0:
        raise NovaError(line, "'math.sqrt' requires a number that is 0 or greater")
    return _math.sqrt(n)
...
def _bi_math_round(args, line):
    n = args[0]
    if not _is_num(n):
        raise NovaError(line, "'math.round' requires a number")
    return int(round(n))
...
def _bi_math_abs(args, line):
    n = args[0]
    if not _is_num(n):
        raise NovaError(line, "'math.abs' requires a number")
    return abs(n)
...
def _bi_math_floor(args, line):
    n = args[0]
    if not _is_num(n):
        raise NovaError(line, "'math.floor' requires a number")
...
def _bi_math_ceil(args, line):
    n = args[0]
    if not _is_num(n):
        raise NovaError(line, "'math.ceil' requires a number")
...
def _bi_math_pow(args, line):
    ...
    if not (_is_num(b) and _is_num(e)):
        raise NovaError(line, "'math.pow' requires two numbers (base and exponent)")
    return b ** e
...
def _bi_random_shuffle(args, line):
    ...
    if not isinstance(xs, list):
        raise NovaError(line, f"'random.shuffle' requires a list — found {nova_str(xs)}")
    out = list(xs)  # copy — the original is untouched
    _random.shuffle(out)
    return out
...
def _bi_text_str(name, v, line):
    if not isinstance(v, str):
        raise NovaError(line, f"'text.{name}' requires text — found {nova_str(v)}")
    return v
...
def _bi_text_split(args, line):
    ...
    if sep == "":
        raise NovaError(line, "'text.split' requires a non-empty separator")
    return s.split(sep)
...
def _bi_text_join(args, line):
    ...
    if not isinstance(xs, list):
        raise NovaError(line, "'text.join' requires a list — give it text.split(...) output first")
...
def _bi_text_replace(args, line):
    ...
    if fra == "":
        raise NovaError(line, "'text.replace' requires a non-empty search text")
...
def _bi_text_at(args, line):
    ...
    n = args[1]
    if not _is_num(n):
        raise NovaError(line, "'text.at' requires a number as the position")
    i = int(n)
    if i < 1 or i > len(s):
        raise NovaError(line, f"position {i} does not exist (the text has only {len(s)} characters)"
                              f" — valid positions are 1 to {max(len(s), 1)}")
    return s[i - 1]
...
def _bi_text_slice(args, line):
    ...
    a, b = args[1], args[2]
    if not (_is_num(a) and _is_num(b)):
        raise NovaError(line, "'text.slice' requires numbers for start/end (1-based, inclusive)")
    a, b = int(a), int(b)
    if a < 1 or b > len(s) or a > b:
        raise NovaError(line, f"slice {a} to {b} reaches outside the text"
                              f" — valid end values are 1 to {max(len(s), 1)}")
    return s[a - 1:b]
...
def _bi_list_xs(name, v, line):
    if not isinstance(v, list):
        raise NovaError(line, f"'list.{name}' requires a list — found {nova_str(v)}")
    return v
...
def _bi_list_sort(args, line):
    ...
    raise NovaError(line, f"'list.sort' cannot mix types ({typer}) — "
                          "give it a list of EITHER numbers OR text")
...
def _bi_list_min(args, line):
    ...
    if not xs:
        raise NovaError(line, "'list.min' requires a non-empty list")
    if not all(_is_num(x) for x in xs):
        raise NovaError(line, "'list.min' requires a list of numbers")
    return min(xs)
...
def _bi_list_max(args, line):
    ...
    if not xs:
        raise NovaError(line, "'list.max' requires a non-empty list")
    if not all(_is_num(x) for x in xs):
        raise NovaError(line, "'list.max' requires a list of numbers")
    return max(xs)
...
def _bi_list_keys(args, line):
    ...
    if not isinstance(bog, dict):
        raise NovaError(line, "'list.keys' requires a dictionary (e.g. from json.parse)"
...
def _bi_list_values(args, line):
    bog = args[0]
    if not isinstance(bog, dict):
        raise NovaError(line, "'list.values' requires a dictionary (e.g. from json.parse)"
                              f" — fandt {nova_str(bog)}")
...
def _make_json_lib():
    ...
    m.funcs["stringify"] = BuiltinFunction("stringify", ["value"], _bi_json_stringify)
    return m
...
def _make_text_lib():
    ...
    m.funcs["contains"] = BuiltinFunction("contains", ["text", "search"], _bi_text_contains)
...
class Interp:
    def __init__(self, seed=None, stdin=None, stdout=None, root_dir=None):
        ...
        self._import_stack = []   # [(abspath, filename)] — circular import detection
    ...
    def readline(self):
        ...
        if line == "":
            raise ExitSignal()  # stdin exhausted — exit gracefully instead of spinning
        return line.rstrip("\r\n")
    ...
    def exec_stmt(self, st, scope):
        ...
        if t == "Assign":
            ...
            if tn == "Var":
                ...
                if not isinstance(obj, ThingInstance):
                    raise NovaError(st.line, "you can only set fields on a thing")
                obj.fields[tgt.name] = val
            else:
                raise NovaError(st.line, "invalid assignment target")
            return
        if t == "AddTo":
            ...
            if isinstance(cur, list):
                ...
                raise NovaError(st.line, f"'add ... to {st.name}' requires a list or a number")
            return
        if t == "TakeFrom":
            cur = scope.get(st.name, st.line)
            val = self.eval(st.expr, scope)
            ...
            if isinstance(cur, list):
                ...
                raise NovaError(st.line, f"'take ... from {st.name}' requires a list or a number")
            return
        if t == "If":
        ...
        if t == "RepeatTimes":
            ...
            if not _is_num(n):
                raise NovaError(st.line, "'repeat N times' requires a number")
        ...
        if t == "RepeatEach":
            ...
            if not isinstance(seq, (list, str)):
                raise NovaError(st.line, "'repeat for each' requires a list or text")
        ...
        if t == "RepeatCounting":
            ...
            if not (_is_num(a) and _is_num(b)):
                raise NovaError(st.line, "'repeat with i from A to B' requires numbers")
        ...
        if t == "UndoStmt":
            ...
            if st.redo:
                if redo:
                    ...
                    raise NovaError(st.line, f"there is nothing to redo for '{st.name}'")
            else:
                if hist:
                    snap = hist.pop()
                    ...
                    raise NovaError(st.line, f"there are no changes to undo for '{st.name}'")
            return
        if t == "Contract":
            ...
            if not ok:
                ...
                raise NovaError(st.line, f"{kind} contract failed — the condition "
                                         f"'{kind}' on this line was not true")
            return
        if t == "RemoveStmt":
            target = st.expr
            if not isinstance(target, ItemAt):
                raise NovaError(st.line, "'remove' supports: remove item N of LIST")
            lst = self.eval(target.e, scope)
            idx = self.eval(target.idx, scope)
            if not isinstance(lst, list) or not _is_num(idx):
                raise NovaError(st.line, "'remove item N of LIST' requires a list and a number")
            i = int(idx)
            if i < 1 or i > len(lst):
                raise NovaError(st.line, f"item {i} does not exist (the list has {len(lst)} items)")
            del lst[i - 1]
            return
        if t == "StoreJson":
            ...
            try:
                ...
                raise NovaError(st.line, f"cannot save to '{path}': {err}")
            return
        if t == "WhenProgramStarts":
            return  # executed last by run()
        raise NovaError(getattr(st, "line", 0), f"unknown statement {t}")
    ...
    def _stdlib_name(self, st):
        ...
        if not ws or ws.pop(0) != "standard":
            raise NovaError(st.line, f"unknown 'use' form: '{st.text}' — write: "
        ...
        if name not in STDLIB_FACTORIES:
            raise NovaError(st.line, f"unknown standard library '{name}' — "
                                     f"available libraries: "
                                     f"{', '.join(sorted(STDLIB_FACTORIES))}")
        return name

    def _load_module(self, path_src, bind_name, line):
        """C05: load (or reuse) a module. Circular import = friendly error."""
        ...
        if any(p == ap for p, _ in self._import_stack):
            ...
            raise NovaError(line, f"circular import: {chain} — modules cannot "
                                  "import each other in a ring; break the chain by "
                                  "moving what they share into a third file")
        ...
        try:
            ...
            raise NovaError(line, f"module file '{path_src}' not found "
                                  f"(searched in '{self._cur_dir}') — check the path and the filename")
        try:
            ...
            for st in stmts:
                if isinstance(st, FuncDef):
                    ...
                    raise NovaError(
                        ...
                        "move the program start to the main program")
            for st in stmts:
                if isinstance(st, (FuncDef, ThingDef, WhenProgramStarts)):
                    continue
    ...
    def _snapshot(self, name, scope):
        ...
        if not scope.has(name):
            return  # first assignment — there is no previous value to snapshot
    ...
    def truth(self, v, node=None):
        ...
        raise NovaError(line, "a condition must be true or false "
    ...
    def eval(self, e, scope):
        ...
        if t == "Field":
            ...
            if isinstance(obj, dict):
                if e.name in obj:
                    return obj[e.name]
                raise NovaError(e.line, f"the dictionary does not have the key '{e.name}'")
            if isinstance(obj, ModuleInstance):
                ...
                raise NovaError(e.line, f"module '{obj.path}' has no '{e.name}'"
                                        f"{_suggest(e.name, known)}"
                                        f" — valid names: {', '.join(known) or '(ingen)'}")
            if obj is NOTHING:
                raise NothingSignal(
                    e.line,
                    f"cannot read the field '{e.name}' of nothing — add '?' "
                    f"if the expression may be nothing (e.g.: the {e.name} of x?), "
                    f"or check the value with 'is nothing' first")
            raise NovaError(e.line, f"cannot read the field '{e.name}' of {nova_str(obj)}")
        if t == "Bin":
        ...
        if t == "ModuleCall":
            ...
            if isinstance(fn, BuiltinFunction):
                if len(args) != len(fn.params):
                    raise NovaError(e.line, f"'{e.mod}.{fn.name}' expects "
                                            f"{len(fn.params)} argument(s), got {len(args)}"
                                            f" — call: {e.mod}.{fn.name}({', '.join(fn.params)})")
        ...
        if t == "RandomBetween":
            ...
            if not (_is_num(a) and _is_num(b)):
                raise NovaError(e.line, "'a random number between A and B' requires numbers")
            return _random.randint(int(a), int(b))
        if t == "NumVal":
        ...
        if t == "CountOf":
            ...
            if hasattr(v, "__len__"):
                return len(v)
            raise NovaError(e.line, f"'how many items are in' requires a list or text")
        if t == "ItemAt":
            lst = self.eval(e.e, scope)
            idx = self.eval(e.idx, scope)
            if not hasattr(lst, "__len__"):
                raise NovaError(e.line, "'item N of' requires a list")
            if not _is_num(idx):
                raise NovaError(e.line, "'item N of' requires a number as the index")
            i = int(idx)
            if i < 1 or i > len(lst):
                raise NovaError(e.line, f"item {i} does not exist (there are {len(lst)} items)"
                                        f" — valid numbers are 1 to {max(len(lst), 1)}")
            return lst[i - 1]
        if t == "FirstItem":
            lst = self.eval(e.e, scope)
        ...
        if t == "ContentsOf":
            ...
            try:
                ...
                raise NovaError(e.line, f"'{path}' does not exist")
            except IsADirectoryError:
                raise NovaError(e.line, f"'{path}' is a directory, not a file")
            except UnicodeDecodeError:
                raise NovaError(e.line, f"'{path}' is not a UTF-8 text file")
            except _json.JSONDecodeError as err:
                raise NovaError(e.line, f"'{path}' contains invalid json (line {err.lineno})")
            except OSError as err:
                raise NovaError(e.line, f"cannot read '{path}': {err}")
        if t == "EveryTurnedInto":
            src = self.eval(e.e, scope)
            thingdef = self.things.get(e.thing)
            if thingdef is None:
                raise NovaError(e.line, f"unknown thing '{e.thing}'")
            out = []
            for item in src:
        ...
        if t == "CopyOf":
            ...
            if isinstance(v, (ModuleInstance, BuiltinFunction, Function)):
                raise NovaError(e.line, f"'a copy of' cannot copy {nova_str(v)} "
                                        "— a module/function is not a value")
            return copy.deepcopy(v)
        raise NovaError(getattr(e, "line", 0), f"unknown expression {t}")

    def _field_err(self, obj, e):
        raise NovaError(e.line, f"{obj.cls} has no field '{e.name}'"
                                f"{_suggest(e.name, obj.fields.keys())}"
                                f" — valid fields: {', '.join(obj.fields.keys()) or '(ingen)'}")
    ...
    def new_thing(self, e, scope):
        ...
        if thingdef is None:
            raise NovaError(e.line, f"unknown thing '{e.cls}'")
    ...
    def call(self, name, args, line):
        ...
        if fn is None:
            raise NovaError(line, f"the function '{name}' does not exist"
                                  f"{_suggest(name, self.funcs.keys())}"
                                  " — define it with 'to <name> ... done'")
        if len(args) != len(fn.params):
            raise NovaError(line, f"'{name}' expects {len(fn.params)} "
                                  f"argument(s), got {len(args)}"
                                  f" — call: {name} with {', '.join(fn.params) or 'nothing'}")
        return self._invoke(fn, args, line)

    def _invoke(self, fn, args, line, parent=None):
        """Run a Function. parent = scope parent (module functions see the module\n        globals; main-program functions see the program globals)."""
        if len(args) != len(fn.params):
            raise NovaError(line, f"'{fn.name}' expects {len(fn.params)} "
                                  f"argument(s), got {len(args)}"
                                  f" — call: {fn.name} with {', '.join(fn.params) or 'nothing'}")
        ...
        for expr_node, ln in pending:
            if not self.truth(self.eval(expr_node, local), expr_node):
                raise NovaError(ln, f"the ensures contract failed in '{fn.name}' — "
                                    "the final state did not satisfy the guarantee")
        return result
    ...
    def eval_bin(self, e, scope):
        ...
        if op in ("gt", "lt", "gte", "lte", "plus", "minus", "times", "divided", "mod"):
            if l is NOTHING or r is NOTHING:
                raise NothingSignal(
                    e.line,
                    "cannot do arithmetic on 'nothing' — add '?' if the expression may "
                    "be nothing (e.g.: n = the number value of answer? + 1), "
                    "or check the value with \'is nothing\' first")
        if op == "gt":  return l > r
        if op == "lt":  return l < r
        ...
        if op == "plus":
            ...
            if _is_num(l) and _is_num(r):
                return l + r
            raise NovaError(e.line, f"cannot add {nova_str(l)} and {nova_str(r)} "
                                    f"— '+' requires two numbers or two texts")
    ...
    def eval_text(self, text, scope):
        ...
        if p.peek().kind != "EOF":
            raise NovaError(p.peek().line,
                            f"unexpected '{p.peek().value}' in {{{text[:30]}}}")
