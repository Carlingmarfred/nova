...
| N04 | Runtime completeness: bigint, dicts/things, `nova_eq` pinning, Optional/`?`, contracts, modules, stdlib v0 parity | P0 | L | N03 | ◐ 2026-08-24: check-patterns ✅ · try/if-it-fails w/ cross-frame unwind ✅ · requires-hoisting + deferred ensures ✅ · things (ctor-func design) ✅ · track/undo/redo history ✅ — **remaining:** modules import/call, stdlib v0, Optional/`?` poisoning, item/length phrase builtins, string interpolation |
| N05 | Differential harness vs Python oracle: corpus runner + output diff (= E06 early cut) | P0 | M | N04 | ◐ **lite harness done 2026-08-24** (`tests/native_diff.py`): 18/18 programs identical (stdout bytes + error core sentences + exit signs) on the supported subset — extends to full corpus as N04f lands. Known cosmetic divergences logged: native errors lack `column N`, oracle wraps with `Parser error —` |
