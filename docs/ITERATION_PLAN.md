...
| N03 | Bytecode compiler + stack VM core: numbers/text/lists, control flow, functions (= E07 front half, pulled early) | P0 | L | N02 | ◐ N03a done 2026-08-24 (value model + `nova_eq` pinning + bigint arith + Python-sign modulo; expression compiler with short-circuit jumps; stack VM; 9 oracle-parity tests) — statements/control flow next |
