...
|---|---|---|
| License | **Apache-2.0** (`LICENSE` at the repo root) — owner decision 2026-08-23. | Permissive with patent protection; the standard choice for language ecosystems. |
| Phrases vs library duplication (Q5) | **Phrases are the primary T1 surface; stdlib namespaces mirror them** (owner decision 2026-08-23). `text.*`/`list.*` are expert ergonomics and must stay semantically identical to their phrase counterparts — one behavior, two spellings. A new stdlib function without a phrase equivalent must justify itself. | The owner wants the language speakable: no memorized command namespaces at T1. |
| Method-call syntax (Q6) | **Verb-first sentences**: `damage the hero with 20` IS the method form; `.name(...)` stays module-only (compact-skin method syntax decided at C10). | Owner decision 2026-08-23 — reads as a sentence, keeps dot-calls unambiguous for modules. |
| Absence-vs-error rule (Q7) | Every builtin declares a family: **Ask** returns `nothing`, **Act** raises. Full table + pin tests in specs/error_handling.md §2.2. | Removes accidental unpredictability; enforced by tests/run_tests.py::test_nothing_rule. |
...
| v0.2 release definition & engine | **v0.2 = the first native interpreter release (ships as 0.20.0)**: Rust bytecode compiler + stack VM behind a swappable-backend boundary (frontend → bytecode → VM, with an IR-like seam so an LLVM/JIT tier can replace or precede the VM without touching the frontend). Dynamic core + opt-in type annotations. First users: developers scripting/CLI automation → stdlib order cli/csv/datetime/regex. Tooling floor: `nova test` + LSP. Uniqueness bets shipping in 0.2: history/undo engine + Flow\<T\>. Python bootstrap stays as a differential-testing oracle, structured for clean removal later. | Owner decision 2026-08-24 — the next release optimizes for "would actually use in the field", not plan-order tooling checkboxes. See ITERATION_PLAN §6 Phase 0.2 (N-series). |
| Integer model (amended) | **Arbitrary-precision integers through 0.2**, identical in both skins and both implementations. This amends the earlier "i32 inferred for int literals" promise: fixed-width integer types arrive with the opt-in annotation layer (post-0.2), keeping debug overflow checks there. | Owner decision 2026-08-24 — oracle↔native behavioral identity (cleaner differential testing) beats premature performance semantics; revisit when annotations land. |
...
- **Integers**: *amended 2026-08-24 — arbitrary precision through v0.2 in BOTH implementations (see decision log); fixed-width types return with the opt-in annotation layer.* Earlier promise kept for reference: fixed-width default (`i32` inferred for int literals); overflow check in
  debug, wrapping in release (opt-in `@checked`). `BigInt` in stdlib.
